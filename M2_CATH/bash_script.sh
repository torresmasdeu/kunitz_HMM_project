...
grep -v -i '1d0d\|2uuy\|2w8x' pdb_codes.txt >pdb_codes_ref.txt

# 2. HMM Generation
# 2.1. Train a HMM profile
grep . msa_ref.fasta |awk '{if (substr($1,1,1)==">") {printf "\n%s ",$1} else {printf "%s",$0}}'|awk '{print $1; print toupper(substr($2,13,54))}'>kunitz_mod.fasta
hmmbuild kunitz_mod.hmm kunitz_mod.fasta
vim kunitz_mod.fasta #to remove 1YC0 sequence
# 2.2. Training set
grep -v -i 'Identifier\|Entity\|,,\|1d0d\|2uuy\|2w8x' pdb_report.csv |cut -d ',' -f 4 |tr -d \" |sort |uniq >training_set.idlist
...
head -n 411 val_set.search |tail -n +18|grep -v inclusion >val_set.out
...
python ../confusion_matrix.py set1.txt 1e-3 > set1.ts
python ../confusion_matrix.py set2.txt 1e-5 > set2.ts
