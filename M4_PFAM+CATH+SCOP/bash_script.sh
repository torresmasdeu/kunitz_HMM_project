...
cat ../PFAM/pdb_codes.txt ../CATH/pdb_codes_ref.txt ../SCOP/pdb_codes.txt |sort |uniq >pdb_codes.txt

# 2. HMM Generation
# 2.1. Train a HMM profile
grep . msa.fasta |awk '{if (substr($1,1,1)==">") {printf "\n%s ",$1} else {printf "%s",$0}}'|awk '{print $1; print toupper(substr($2,13,56))}' >kunitz_mod.fasta
vim kunitz_mod.fasta #to remove 1TFX, 1YC0 and 4ISL sequences
hmmbuild kunitz_mod.hmm kunitz_mod.fasta
# 2.2. Training set
cat ../PFAM/training_set.idlist ../CATH/training_set.idlist ../SCOP/training_set.idlist |sort |uniq >training_set.idlist
...
head -n 403 val_set.search|tail +18 |grep -v inclusion >val_set.out
...
python ../confusion_matrix.py set1.txt 1e-4 > set1.ts
python ../confusion_matrix.py set2.txt 1e-3 >set2.ts
cat val_set.randomp val_set.randomn > val_set.txt
python ../confusion_matrix.py val_set.txt 5.5e-4 >val_set.ts